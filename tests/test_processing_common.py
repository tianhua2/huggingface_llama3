...
class ProcessorTesterMixin:
    ...
    def prepare_text_inputs(self, batched=False):
        if batched:
            return ["lower newer", "upper older longer string"]
        return "lower newer"
    ...
    def test_unstructured_kwargs_batched(self):
        ...
        input_str = self.prepare_text_inputs(batched=True)
        image_input = self.prepare_image_inputs(batched=True)
        inputs = processor(
            text=input_str,
            images=image_input,
            return_tensors="pt",
